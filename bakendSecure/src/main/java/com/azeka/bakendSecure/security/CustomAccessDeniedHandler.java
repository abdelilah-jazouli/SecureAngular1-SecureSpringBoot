...
public class CustomAccessDeniedHandler implements AccessDeniedHandler {
    ...
    final static  Logger logger = LoggerFactory.getLogger(CustomAccessDeniedHandler.class); 
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (accessDeniedException instanceof CsrfException && !response.isCommitted()) {
            ...
            logger.info("BachendSecure : Removing the CSRF session cookie so that client knows it's time to obtain a new CSRF token" );
        }
